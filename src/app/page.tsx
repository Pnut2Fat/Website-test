...
export default function HomePage() {
  return (
    <>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
          {perks.map(({ icon, title, body }) => (
            <div key={title} className="text-center">
              ...
              <h3 className="font-heading text-lg font-semibold text-brown mb-2">{title}</h3>
      ...
      {/* Divider */}
      <div className="h-px bg-cream-dark max-w-6xl mx-auto" />

      {/* Story teaser */}
      <section className="py-16 sm:py-20 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto">
        <div className="max-w-2xl">
          <p className="text-amber text-sm font-semibold uppercase tracking-widest mb-3">Our Story</p>
          <h2 className="font-heading text-3xl sm:text-4xl font-bold text-brown mb-5">
            Real land. Real chickens.
            <br />Real eggs.
          </h2>
          <p className="text-brown-light text-base leading-relaxed mb-6">{farmInfo.story[0]}</p>
          <Link
            href="/about"
            className="inline-block text-green font-semibold border-b-2 border-amber hover:text-green-light transition-colors"
          >
            Learn more about us →
          </Link>
        </div>
      ...
      {/* Products preview */}
      <section className="bg-cream-dark py-16 sm:py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-end justify-between mb-10">
            <div>
              ...
              <h2 className="font-heading text-3xl sm:text-4xl font-bold text-brown">Our Eggs</h2>
            </div>
            <Link
              href="/products"
              className="hidden sm:inline text-sm text-green font-semibold hover:text-green-light transition-colors"
            >
              See all →
            </Link>
          </div>
          ...
          <div className="mt-8 text-center sm:hidden">
            <Link href="/products" className="text-sm text-green font-semibold hover:text-green-light">
              See all products →
            </Link>
          </div>
        </div>
